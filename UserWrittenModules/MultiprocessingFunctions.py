...
#  you should change the below credentials in order to use your dialogflow account!. please do the necessorry
...
# Dialogflow credentials are rmoved from the repo for safety issues (and since we are still working ion it ). If you are in need of the API keys for rhe same, do contact at hi@niran.dev
DIALOGFLOW_PROJECT_ID = 'reda-catqul'
...
def DialogflowSocket(queryText):
    ...
    #print("***PRINT END***")
    magret = [str(response.query_result.fulfillment_text), str(response.query_result.query_text), str(response.query_result.intent.display_name), str(response.query_result.intent_detection_confidence), str(response.query_result.action) ]
    return(magret)
