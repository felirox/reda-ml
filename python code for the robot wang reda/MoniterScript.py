...
while cap.isOpened():
    ...
    should_i_send_image, flagging = False, False
    ...
    for r in red_boxes:
        ...
        if not flagging:
            if not yell.is_alive():
                yell = thread.Thread(target=multi.MakeAwareness, args=(str(yelling[randint(0, 9)]),))
                yell.start()
                should_i_send_image = True
            flagging = True
        cv2.putText(img, "ALERT! Physical distancing is violated", (int(A), int(B) - 5), cv2.FONT_HERSHEY_PLAIN, 0.5,
                    [0, 0, 255], 1)
        cv2.rectangle(img, (A, B), (A + C, B + D), [0, 0, 255], 2)

    if should_i_send_image:
        should_i_send_image = False
        count += 1
        file_name = str(count)+".jpg"
        thread.Thread(target=multi.upload_image_to_server_and_increment_count, args=(img, file_name,)).start()
